...
local function parse_attributes(body, start)
  ...
  while i <= #body do
    ...
    if not finish then return nil, "unterminated attribute " .. tostring(name) end
...
local function xml_tree(xml)
  if type(xml) ~= "string" then return nil, "XML must be a string" end
  ...
  while position <= #xml do
    ...
    elseif xml:sub(opening, opening + 1) == "<!" then
      ...
      while i <= #xml do
        ...
        elseif c == "]" and brackets > 0 then brackets = brackets - 1
        elseif c == ">" and brackets == 0 then finish = i; break end
        i = i + 1
      end
      if not finish then return nil, "unterminated XML declaration or doctype" end
      position = finish + 1
    else
      local finish = tag_end(xml, opening + 1)
      if not finish then return nil, "unterminated XML tag" end
      ...
      if body:match("^%s*/") then
        ...
        if not name then return nil, "tag without a name" end
  ...
  if #stack > 0 then return nil, "unclosed XML tag " .. tostring(stack[#stack].tag) end
  if not root then return nil, "XML has no root element" end
  return root
end
...
local function child(node, name)
  ...
  for _, item in ipairs(node.children or {}) do
    if item.tag == name then return item end
  end
end
...
local function children(node, name)
  ...
  if node then
    name = local_name(name)
    for _, item in ipairs(node.children or {}) do
      if item.tag == name then result[#result + 1] = item end
    end
  end
  return result
end
...
local function number(value, default)
  ...
  local sign = 1
  if value:sub(1, 1) == "-" then sign, value = -1, value:sub(2) end
  local hex = value:match("^0[xX](.*)$")
  local result
  if hex then result = tonumber(hex, 16) else result = tonumber(value) end
...
local function clone(node)
  ...
  for key, value in pairs(node.attrs or {}) do result.attrs[key] = value end
  for _, item in ipairs(node.children or {}) do result.children[#result.children + 1] = clone(item) end
  return result
end

local merge_containers = {
  registers = true, fields = true, peripherals = true, enumeratedValues = true,
  defaultRegisterProperties = true, registerProperties = true, addressBlock = true,
}

local function node_identity(node)
  if node.attrs and node.attrs.name then return node.attrs.name end
  if node.tag == "register" or node.tag == "cluster" or node.tag == "field" or node.tag == "peripheral" or node.tag == "enumeratedValue" then
...
local function merge(base, overlay)
  ...
  for key, value in pairs(overlay.attrs or {}) do result.attrs[key] = value end
  if trim(overlay.text) ~= "" then result.text = overlay.text end
  for _, incoming in ipairs(overlay.children or {}) do
    ...
    if match then result.children[match] = merge(result.children[match], incoming)
    else result.children[#result.children + 1] = clone(incoming) end
  end
  return result
end
...
local function resolve(map, node, seen)
  ...
  if not source then return clone(node) end
  seen = seen or {}
  ...
  if seen[key] then return clone(node) end
  seen[key] = true
  local source_node = map[source] or map[reference_name(source)]
  local result = source_node and resolve(map, source_node, seen) or { tag = node.tag, attrs = {}, children = {}, text = "" }
  seen[key] = nil
...
local function parse_index_list(value_text, count)
  ...
  if not value_text or value_text == "" then
    local result = {}
    for i = 0, count - 1 do result[#result + 1] = tostring(i) end
    return result
  end
  local result = {}
  for part in value_text:gmatch("[^,%s]+") do
    ...
    if first and last and tonumber(first) and tonumber(last) then
      ...
      for i = a, b, step do result[#result + 1] = tostring(i) end
    elseif first and last and #first == 1 and #last == 1 then
      local a, b = first:byte(), last:byte()
      local step = a <= b and 1 or -1
      for i = a, b, step do result[#result + 1] = string.char(i) end
    else
      result[#result + 1] = part
    end
  end
  while #result < count do result[#result + 1] = tostring(#result) end
  return result
end
...
local function substitutions(node, callback)
  ...
  for i = 1, count do
    ...
    local function replace(value_text)
      if not value_text then return value_text end
    ...
    if item_name then item_name.text = replace(item_name.text) end
    local item_display = child(item, "displayName")
    if item_display then item_display.text = replace(item_display.text) end
...
local function properties(node, inherited)
  local result = {}
  for key, val in pairs(inherited or {}) do result[key] = val end
  ...
  if source then
    for _, key in ipairs({ "size", "access", "resetValue", "resetMask" }) do
      ...
      if raw ~= nil then result[key] = (key == "access" and raw) or number(raw) end
    end
  end
  for _, key in ipairs({ "size", "access", "resetValue", "resetMask" }) do
...
local function field_model(node, inherited_access, field_map, register_name)
  ...
  if bit_offset == nil or bit_width == nil then
    ...
    if lsb ~= nil and msb ~= nil then bit_offset, bit_width = lsb, msb - lsb + 1 end
  end
  if bit_offset == nil or bit_width == nil then
    local range = text(effective, "bitRange")
    local msb, lsb = range and range:match("^%[%s*(%d+)%s*:%s*(%d+)%s*%]$")
    if msb and lsb then bit_offset, bit_width = tonumber(lsb), tonumber(msb) - tonumber(lsb) + 1 end
  end
  ...
  for _, enum in ipairs(result.enumeratedValues) do
    if enum.name then result.enumeratedValuesByName[enum.name] = enum end
  end
  return result
end
...
local function make_register(effective, peripheral, offset, cluster, dim_index, field_map)
  ...
  for _, field in ipairs(children(child(effective, "fields"), "field")) do
    substitutions(field, function(item)
      ...
      if normalized.name then result.fields_by_name[normalized.name] = normalized end
    end)
  end
  return result
end
...
local function build_model(root)
  ...
  for _, key in ipairs({ "size", "access", "resetValue", "resetMask" }) do
    ...
    if raw ~= nil then defaults[key] = key == "access" and raw or number(raw) end
  end
  ...
  for _, node in ipairs(peripheral_nodes) do
    ...
    if name then peripheral_map[name] = node end
  end
  ...
  if cpu then
    for _, key in ipairs({ "name", "revision", "endian", "mpuPresent", "fpuPresent", "fpuDP", "dspPresent", "icachePresent", "dcachePresent", "vtorPresent", "nvicPrioBits", "vendorSystickConfig" }) do
      local raw = text(cpu, key)
      if raw ~= nil then model.cpu[key] = tonumber(raw) or raw end
    end
  end

  local address_unit_bits = model.addressUnitBits or 8
  local address_unit_scale = address_unit_bits / 8
  if address_unit_scale <= 0 then address_unit_scale = 1 end
              ...
              for _, field in ipairs(children(child(item, "fields"), "field")) do
                ...
                if field_name then
                  ...
                  if local_names then
                    ...
                    if not field_map[field_name] then field_map[field_name] = field end
                  end
                end
              end
            else
      ...
      for _, raw_item in ipairs(container or {}) do
        if raw_item.tag == "register" then
          ...
          substitutions(effective, function(item, index, zero_index, increment)
            ...
            if not peripheral.registers_by_name[item_name] then peripheral.registers_by_name[item_name] = normalized end
          end)
...
function M.parse(xml)
  ...
  if root.tag ~= "device" then return nil, "SVD root must be <device>" end
...
function M.load_file(path, callback)
  ...
  if callback then callback(model, parse_err) end
