...
local function storage_path()
  ...
  if root == '' then root = vim.fn.stdpath('data') end
...
local function load_store()
  if state.store then return state.store end
  state.store = {}
...
local function save_store()
  ...
  if not ok then notify('could not save the selected debug target', vim.log.levels.ERROR) end
  return ok
end
...
local function add_config(configs, seen, config)
  if type(config) ~= 'table' or not config.name then return end
...
local function configs_for(_, launch_path)
  ...
  if ok_vscode then
    ...
    if ok_configs and type(launch_configs) == 'table' then
      for _, config in ipairs(launch_configs) do add_config(configs, seen, config) end
    end
  end
  local filetype_configs = d.configurations[vim.bo.filetype] or {}
  for _, config in ipairs(filetype_configs) do add_config(configs, seen, config) end
...
local function format_item(config)
  return string.format('%s  [%s / %s]', tostring(config.name), tostring(config.type or '?'), tostring(config.request or 'launch'))
end
...
function P.select(callback)
  ...
  if current then prompt = prompt .. ' (current: ' .. tostring(current.name) .. ')' end
...
function P.start()
  ...
  if d.session() then return d.continue() end
  local root, launch_path = project()
  if not root then
    notify('no .vscode/launch.json found from the current directory', vim.log.levels.ERROR)
    return nil
  ...
  return P.select(function(selected)
    if selected then d.run(selected) end
  end)
end
