...
local dap = {
  ...
  run = function(config) run_config = config end,
  continue = function() end,
}
package.loaded.dap = dap
package.preload['dap.ext.vscode'] = function()
  return { getconfigs = function() return configs end }
end
...
check('selection is stored outside workspace', vim.fn.filereadable(storage) == 1 and vim.fn.filereadable(tmp .. '/targets.json') == 0)
run_config = nil
target.start()
...
if storage_exists then vim.fn.writefile(storage_lines, storage) else vim.fn.delete(storage) end
